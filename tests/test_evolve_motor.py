...
def _ev(c, root, goal_fn=_ok_goal):
    # inyecta goal + ensemble (sin API); run_tests=False (no corre la suite anidada)
    return evaluate(c, root=root, run_tests=False, goal_fn=goal_fn,
                    ensemble_fn=lambda ch: True)
...
def test_evaluate_rejects_goal_misalign(tmp_path):
    ...
    r = _ev(c, tmp_path, goal_fn=_bad_goal)
...
def test_self_evolve_applies_green_winner(tmp_path):
    ...
    res = self_evolve(candidates=[good, bad], root=tmp_path, do_apply=True, audit=False,
                      evaluate_fn=lambda c: _ev(c, tmp_path))
