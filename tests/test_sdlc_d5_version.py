...
quedaba sin version propia. `server_version()` cae al `version = "..."` de [project] en el
...
def test_mmorch_version_cae_a_pyproject_sin_metadata(monkeypatch):
    ...
    assert M.server_version() == _pyproject_version()
...
def test_mmorch_version_usa_metadata_cuando_existe(monkeypatch):
    ...
    assert M.server_version() == "9.9.9-test"


def test_server_expone_version_no_vacia():
    assert M.mcp._mcp_server.version and M.mcp._mcp_server.version == M.server_version()
