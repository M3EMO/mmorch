...
def test_perfect_verifier(monkeypatch):
    ...
    def _c(model, messages, **kw):
        ...
        good = "2+2=4" in art
        # labels anclados (W5.3): desde ronda 2 el legacy {"passed":true} ya no
        # aprueba (D-adv2) — el mock habla el contrato vigente.
        return _Res('{"verdict": "correcto", "refutations": []}' if good
                    else '{"verdict": "incorrecto_grave", "refutations": ["mal"]}')
...
def test_sycophant_verifier_false_pass(monkeypatch):
    ...
    monkeypatch.setattr(A, "call", lambda *a, **k: _Res('{"verdict": "correcto", "refutations": []}'))
...
def test_ranking_best_first(monkeypatch):
    ...
    def _c(model, messages, **kw):
        if "gemini" in model:
            ...
            return _Res('{"verdict": "correcto"}' if good else '{"verdict": "incorrecto_grave"}')
        return _Res('{"verdict": "correcto"}')  # deepseek aprueba todo
    monkeypatch.setattr(A, "call", _c)
