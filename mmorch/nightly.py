...
Registrar:  schtasks /Create /TN mmorch-nightly /SC DAILY /ST 02:10 /F
            ...
            (compat: scripts/nightly.py es un shim que llama aca)
"""
import io
import json
import os
...
# la raiz operativa (repo sobre el que evoluciona + estado) resuelve via
# paths.home() — default el checkout, overrideable con MMORCH_HOME (W2.1).
from mmorch.paths import home as _mmorch_home  # noqa: E402
...
def main() -> None:
    ...
    if time.localtime().tm_yday % 2 == 0:
        _ar_t, _ar_s = "mmorch/prompts/coder_prompt.txt", "scripts/score_coder_prompt.py"
    else:
        _ar_t, _ar_s = "mmorch/prompts/idea_madurar.txt", "scripts/score_idea_maturation.py"
