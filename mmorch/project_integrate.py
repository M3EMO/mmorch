...
def _default_gen(gen_model: str, repo: str, task: str = "", units_by_name: dict | None = None):
    from .providers import call
    from .textutil import extract_fence
    _units_by_name = units_by_name if units_by_name is not None else {}
    ...
    def gen(unit: dict, feedback: str) -> str:
        ...
        # A/B 2026-09-10 (chatbot, port a Java): el coder de `bot` nunca vio Msg.java y llamo
        # Msg.text(x) como setter; Msg lo definia como getter -> 19 errores de compilacion en
        # la integracion. Las unidades se construian ciegas a sus deps. Ahora el coder ve el
        # codigo REAL de las unidades de las que depende (ya construidas, en disco) -- lo mismo
        # que hace el brazo B a mano: cada archivo se escribe viendo los anteriores.
        siblings, budget = [], 40000
        for d in unit.get("deps", []) or []:
            du = _units_by_name.get(d)
            if not du:
                continue
            dpath = _safe_target(repo, du)
            if os.path.isfile(dpath):
                try:
                    body = open(dpath, encoding="utf-8").read()[:budget]
                except (OSError, UnicodeDecodeError):
                    continue
                budget -= len(body)
                siblings.append(f"// {_file_of(du)} (dependency `{d}`, already built -- use its REAL API)\n{body}")
                if budget <= 0:
                    break
        deps_ctx = ("\nDEPENDENCIES (already built; call them exactly as defined):\n```\n"
                    + "\n\n".join(siblings) + "\n```\n") if siblings else ""
...
def build_project(task: str, repo: str, *, external_test: str | None,
    ...
    _units_reg: dict = {}   # name -> unit; lo llena plan_fn, lo lee el coder para ver sus deps
    gen = gen or _default_gen(gen_model, repo, task, _units_reg)
    ...
    _inner_plan = plan or _default_plan

    def plan_fn(t: str, ext: str | None) -> list[dict]:
        units = _inner_plan(t, ext)
        for u in units or []:
            if u.get("name"):
                _units_reg[u["name"]] = u
        return units
