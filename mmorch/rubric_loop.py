...
# --------------------------------------------------------------------------- #
# commit-then-reveal (graft Estudio wiki-study modo 11, 2026-08-07): la rubrica
# se ESCRIBE Y COMPROMETE (sha256) ANTES de ver el intento/respuesta — elimina
# el sesgo de rubric-fitting (armar la vara despues de ver el salto). Reveal
# verifica el hash (tamper-evident); el humano puede EDITAR la rubrica revelada
# si es injusta y re-puntuar — las ediciones quedan registradas, no ocultas.
# --------------------------------------------------------------------------- #
def commit_rubric(task: str, criteria: list[dict], *, store_dir: str | None = None) -> dict:
    """Escribe la rubrica OCULTA a disco y devuelve solo {id, sha256} — el
    compromiso publico. El contenido no se muestra hasta reveal_rubric()."""
    import hashlib
    ...
    rid = h[:12]
    (d / f"{rid}.json").write_text(payload, encoding="utf-8")
    return {"id": rid, "sha256": h}


def reveal_rubric(rid: str, *, store_dir: str | None = None,
                  edits: list[dict] | None = None) -> dict:
    """Revela la rubrica comprometida, verificando el hash (tamper-evident).
    `edits` opcional: criterios reemplazados por el humano si la vara era injusta
    — el resultado registra original + editada, nunca pisa el compromiso.
    Devuelve {task, criteria, sha256_ok, edited, original_criteria}."""
    import hashlib
    import json as _json
    from pathlib import Path
    ...
    out = {"task": data["task"], "criteria": data["criteria"], "sha256_ok": ok,
           "edited": False, "original_criteria": data["criteria"]}
    if edits:
        out["criteria"] = edits
        out["edited"] = True
    return out
